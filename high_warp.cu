...
__global__ void ellipseResponseMapKernelBetter(float *Mg, float *Og, float *gradX, float *gradY, float *gradients, int *idxs, int maxThreads){
    ...
    if(y < img_height && y >= 0 && x < img_width && x >= 0){
        if(gradients[idx] != 0){
            ...
            printf("%d/%d - (%d, %d) spawned unnecessarily\n", fidx, maxThreads, y, x);
        }
...
__global__ void organise_indices(int* non_zeros_scanned, int *store_results){
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
    if(y != 0 || x != 0){
...
driverCuda(){
    ...
    */
    ...
    organise_indices<<<gridDim, blockDim>>>(thrust::raw_pointer_cast(non_zero_grads.data()), store_results);
    ...
    int threadsPerBlock = 64; int numBlocks = (num_nonzero_grads_c + (threadsPerBlock - 1)) / threadsPerBlock;
    ellipseResponseMapKernelBetter<<<numBlocks, threadsPerBlock>>>(Mg_d, Og_d, gradX_d, gradY_d, nms_gradients_d,
                                                                   store_results, non_zero_grads[img_width_c * img_height_c - 1]);
